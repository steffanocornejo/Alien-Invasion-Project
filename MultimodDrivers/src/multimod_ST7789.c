...
void ST7789_Select(void) {
    // write to PF4 and check multimod for pins to deselect/ reselct SPI pinsz
    GPIOPinWrite(GPIO_PORTF_BASE, ST7789_CS_PIN, 0);
...
void ST7789_Deselect(void) {
    GPIOPinWrite(GPIO_PORTF_BASE, ST7789_CS_PIN, ST7789_CS_PIN);
}
...
void ST7789_SetData(void) {
    ...
    GPIOPinWrite(GPIO_PORTF_BASE, ST7789_DC_PIN, 0xFF);
    }
...
void ST7789_SetCommand(void) {
    GPIOPinWrite(GPIO_PORTF_BASE, ST7789_DC_PIN, 0);
    }
...
uint8_t ST7789_ReadRegister(uint8_t data) {
    ...
    uint8_t output;
    output = SPI_ReadSingle(SPI_A_BASE);
...
void ST7789_Init() {
    ...
    ST7789_WriteCommand(ST7789_CASET_ADDR);
    delay_ms(120);
    ST7789_WriteData(0x00);   // Start Col High
    delay_ms(120);
    ST7789_WriteData(0x00);   // Start Col Low
    delay_ms(120);
    ST7789_WriteData(0xFF);   // End Col High
    delay_ms(120);
    ST7789_WriteData(0xFF);   // End Col Low (239)

    // edit raset (row origin)
    ST7789_WriteCommand(ST7789_RASET_ADDR);
    delay_ms(120);
    ST7789_WriteData(0x00);   // Start Row High
    delay_ms(120);
    ST7789_WriteData(0x14);
    delay_ms(120);
    ST7789_WriteData(((Y_MAX+20) >> 8) & 0x0F);
    delay_ms(120);
    ST7789_WriteData((Y_MAX+20) & 0x0F);
...
void ST7789_DrawSprite(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* image) {
    // 1. Bounds check
    if ((x >= X_MAX) || (y >= Y_MAX)) return;

    // 2. Loop through every pixel in the array
    uint32_t index = 0;
    for (uint16_t row = 0; row < h; row++) {
        for (uint16_t col = 0; col < w; col++) {

            // Get the color from the array
            // Note: I removed the '~' inversion. Add it back if your colors look wrong.
            uint16_t color = ~image[index++];

            // 3. TRANSPARENCY CHECK
            // If the pixel is NOT black (0x0000), draw it.
            // If it IS black, do nothing (skip it), revealing the background.
            if (color != 0xFFFF) {
...
void ST7789_EraseSprite(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* image) {
    if ((x >= X_MAX) || (y >= Y_MAX)) return;
    ...
    for (uint16_t row = 0; row < h; row++) {
        for (uint16_t col = 0; col < w; col++) {
            ...
            // LOGIC:
            // If the sprite pixel is NOT transparent (meaning it's part of the ship),
            // We draw BLACK over it to erase it.
            // If the sprite pixel IS transparent (0x0000), we do nothing.
            if (color != 0xFFFF) {
                ST7789_DrawPixel(x + col, y + row, 0xFFFF); // Draw Black
            }
        }
    }
}
